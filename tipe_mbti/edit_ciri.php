<?php 
    session_start();
    require_once '../controller/ciri.php';

    $idciri = dekripsi($_GET['id']);
    $data = query("SELECT * FROM ciri_mbti WHERE id_ciri = $idciri")[0];
                ...
                <div class="contents" style="margin: 75px 0; padding: 10px 40px;">
                    ...
                    <form method="post" action="">
                        ...
                        <div class="mb-4 mt-2 row ms-5">
                            <label for="inputEmail" class="col-sm-3 me-0 col-form-label">Ciri-Ciri</label>
                            <div class="col-sm-6">
                                <textarea type="text" style="border-color: black;" class="form-control" id="inputGejala"
                                    name="ciri" rows="6"><?= $data['ciri']; ?></textarea>
                            </div>
                        </div>
...
<?php 
  if(isset($_POST['submit'])) {
    if (update($_POST) > 0) {
      $_SESSION["berhasil"] = "Data Ciri-Ciri MBTI Berhasil Diubah!";
      ...
      ";
        ...
        echo "<script>
                Swal.fire(
                    'Gagal!',
                    'Data Ciri-Ciri MBTI Gagal Diubah',
                    'error'
                )
            </script>";
        exit();
